...
const PriceCard = ({ card, id, toggleMonthly }) => {
  const containerNature = card.class === 'transparent'? style.transparent: style.colored;
  const buttonNature = card.class === 'transparent'? style.white : style.blue;
  return (
    
      <section
        key={id}
        className={`${style.cardContainer} ${containerNature}`}
      >
        <div className={style.innerCardContainer}>
          ...
          <button
            className={`${style.btn} ${buttonNature}`}
          >
            {card.button}
          </button>
          </Link>
