...
describe("user authentication", () => {
    ...
    it("should not login a user", async () => {
        ...
        assert.equal(res2.status, 200);
        assert.notExists(res2.body.token);
        assert.equal(Object.keys(res2.body.user).length, 0);
        assert.include(res2.body.message, "Pls, kindly check your e-mail to complete your registration");
        docmaker.addEndpoint(res2);
    });
});
